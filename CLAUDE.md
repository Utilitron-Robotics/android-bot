...
```bash
# Open relay_app/ folder in Android Studio, then Run
# Or via command line:
cd relay_app
...
2. Open in Android Studio OR run `./gradlew assembleDebug`
3. Install on tablet: `adb install -r app/build/outputs/apk/debug/app-debug.apk`
4. Monitor logs: `adb logcat | grep -E "Relay|Grpc|Robot"`
...
- Relay: Android Studio Run button
